...
import { OfficeMangerColumn } from 'models/OfficeManager';
import React, { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { v4 as uuid } from 'uuid';

const dataSource: OfficeMangerColumn[] = [];
for (let i = 0; i < 15; i++) {
  dataSource.push({
    id: uuid(),
    title: 'Lyon',
          ...
          <Grid container spacing={2}>
            {Object.keys(dataDetails).map((i) => (
              <React.Fragment key={i}>
                <Grid item xs={3}>
                  <Typography variant="body2">{t(`destinations:${i}`)}:</Typography>
                </Grid>
                <Grid item xs={9}>
...
export default React.memo(TableOfficesManager);
