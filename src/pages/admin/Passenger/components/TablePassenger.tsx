...
function TablePassenger({ onSelect }: TablePassengerProps) {
  ...
  const actions: ActionItem<PassengerTypeColumn>[] = [
    ...
    { id: uuid(), label: 'block', icon: <RemoveCircleOutlineIcon sx={{ color: '#FF2727' }} />, onClick: handleCancel, color: '#FF2727' },
  ];
  ...
  return (
    <Box my="24px">
      <AntTable
        rowSelection={{
          type: 'checkbox',
          onChange: (selectedRowKeys: React.Key[], selectedRows: PassengerTypeColumn[]) => onSelect?.(selectedRows),
        }}
        columns={columns}
